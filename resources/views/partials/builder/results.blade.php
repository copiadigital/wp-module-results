<div class="section results" x-data="{ activeTab: 0 }">
  @if($results)
    <ul class="results__nav table-tabs flex flex-wrap list-none border-b border-gray-200 px-0" role="tablist">
      @foreach ($results as $key => $item)
        @if($item['years'])
          <li class="results__nav-item" role="presentation">
            <button
              type="button"
              class="results__nav-link px-10 py-10 border-b-2 transition-colors"
              :class="activeTab === {{ $key }} ? 'border-current font-semibold' : 'border-transparent hover:border-gray-300'"
              @click="activeTab = {{ $key }}"
              role="tab"
    ...
    <div class="results__tab-content">
      @foreach($results as $key => $item)
        ...
        >
          {{-- Links --}}
          @if($item['results'])
            <table class="results__table w-full border-collapse">
              <thead class="results__table-head">
                <tr>
                  <th class="results__table-head-date w-[20%] p-20">Date</th>
                  <th class="results__table-head-title w-1/2 p-20">Title</th>
                  <th class="results__table-head-press-release text-center w-[10%] p-20">Press release</th>
                  <th class="results__table-head-presentation text-center w-[10%] p-20">Presentation</th>
                  <th class="results__table-head-report text-center w-[10px] p-20">Report</th>
                </tr>
              </thead>
              <tbody class="results__table-body">
                @foreach($item['results'] as $result)
                  <tr>
                    <td class="results__table-body-date block md:table-cell text-sm text-gray-600 p-20">{!! $result['date'] !!}</td>
                    <td class="results__table-body-title block md:table-cell font-semibold text-gray-600 p-20">{!! $result['title'] !!}</td>
                    <td class="results__table-body-press-release md:text-center p-20">
                      @if($result['press_release'])
                        <a href="{{ $result['press_release']['url'] }}" target="_blank" aria-label="{{ $result['press_release']['title'] }}">
                          ...
                          <span class="inline-block md:hidden">Press release</span>
                        </a>
                      @endif
                    </td>
                    <td class="results__table-body-presentation md:text-center p-20">
                      @if($result['presentation'])
                        <a href="{{ $result['presentation']['url'] }}" target="_blank" aria-label="{{ $result['presentation']['title'] }}">
                          ...
                          <span class="inline-block md:hidden">Presentation</span>
                        </a>
                      @endif
                    </td>
                    <td class="results__table-body-report md:text-center p-20">
                      @if($result['report'])
                        <a href="{{ $result['report']['url'] }}" target="_blank" aria-label="{{ $result['report']['title'] }}">
                          ...
                          <span class="inline-block md:hidden">Download</span>
                        </a>
                      @endif
                    </td>
                  </tr>
